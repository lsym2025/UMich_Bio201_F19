...
# p value is relatively large here, so there might not be practical significant difference between week1 and week3 when consumed once daily                                                                                                                                                                                                                                                                                                                                                                                                                                                       
...
breath_wkly <- read_delim("Lab5/curated_data/breath_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
  rename_all(tolower) %>%
  mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
...
methane <- breath_wkly %>%
  select(-starts_with("h2"), -starts_with("co2"), -starts_with("n"),-ends_with("median")) %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS" | supplement_consumed == "LOODAT", 
         quantity_compliant == "yes") %>%
  na.omit(ch4_mean) 

plot_methane<-methane %>%
...
save_plot(filename = "Lab5/figures/methane.pdf",
          plot = plot_methane)

#seems to increase base on the plot
```
...
save_plot(filename = "Lab5/figures/acetate.pdf",
          plot = plot_ace)
...
save_plot(filename = "Lab5/figures/propionate.pdf",
          plot = plot_pro)
...
```{r}
 ph_wkly <- read_delim("Lab5/curated_data/pH_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
  rename_all(tolower) %>%
  mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
...
save_plot(filename = "Lab5/figures/pH.pdf",
          plot = plot_ph)

#seems to decrease base on the plot
...
shapiro.test(meth_df1$ch4_mean)

meth_df2 <- methane %>%
  filter(supplement_consumed =="BRMPS",study_week == "week1")
shapiro.test(meth_df2$ch4_mean) 

ggplot(meth_df1, aes(x=ch4_mean)) +
  geom_histogram() 

qqnorm(meth_df1$ch4_mean); qqline(meth_df1$ch4_mean)

ggplot(meth_df2, aes(x=ch4_mean)) +
  geom_histogram() 

qqnorm(meth_df2$ch4_mean); qqline(meth_df2$ch4_mean)

## This histogram shows a rough bell curve, in combination with our large sample size we are okay with proceeding with a t-test. Although the p-value is small.

var.test(x = meth_df1$ch4_mean, 
         y = meth_df2$ch4_mean, 
         alternative = "two.sided")
## Variance not equal
...
ace_df1 <- acetate_long %>%
  filter(supplement_consumed =="BRMPS",study_week == "week3") 
shapiro.test(ace_df1$acetate_mean)

ace_df2 <- acetate_long %>%
  filter(supplement_consumed =="BRMPS",study_week == "week1")
shapiro.test(ace_df2$acetate_mean) 

ggplot(ace_df1, aes(x=acetate_mean)) +
  geom_histogram() 

qqnorm(ace_df1$acetate_mean); qqline(ace_df1$acetate_mean)

ggplot(ace_df2, aes(x=acetate_mean)) +
  geom_histogram() 

qqnorm(ace_df2$acetate_mean); qqline(ace_df2$acetate_mean)

## This histogram shows a rough bell curve, in combination with our large sample size we are okay with proceeding with a t-test. Although the p-value is small.

var.test(x = ace_df1$acetate_mean, 
         y = ace_df2$acetate_mean, 
         alternative = "two.sided")
...
pro_df1 <- propionate_long %>%
  filter(supplement_consumed =="BRMPS",study_week == "week3") 
shapiro.test(pro_df1$propionate_mean)

pro_df2 <- propionate_long %>%
  filter(supplement_consumed =="BRMPS",study_week == "week1")
shapiro.test(pro_df2$propionate_mean) 

ggplot(pro_df1, aes(x=propionate_mean)) +
  geom_histogram() 

qqnorm(pro_df1$propionate_mean); qqline(pro_df1$propionate_mean)

ggplot(pro_df2, aes(x=propionate_mean)) +
  geom_histogram() 

qqnorm(pro_df2$propionate_mean); qqline(pro_df2$propionate_mean)

## This histogram shows a rough bell curve, in combination with our large sample size we are okay with proceeding with a t-test. Although the p-value is small.

var.test(x = pro_df1$propionate_mean, 
         y = pro_df2$propionate_mean, 
         alternative = "two.sided")
```
...
ph_df1 <- ph%>%
  filter(supplement_consumed =="BRMPS",study_week == "week3") 
shapiro.test(ph_df1$ph_mean)

ph_df2 <- ph %>%
  filter(supplement_consumed =="BRMPS",study_week == "week1")
shapiro.test(ph_df2$ph_mean) 

ggplot(ph_df1, aes(x=ph_mean)) +
  geom_histogram() 

qqnorm(ph_df1$ph_mean); qqline(ph_df1$ph_mean)

ggplot(ph_df2, aes(x=ph_mean)) +
  geom_histogram() 

qqnorm(ph_df2$ph_mean); qqline(ph_df2$ph_mean)

## This histogram shows a rough bell curve, in combination with our large sample size we are okay with proceeding with a t-test. Although the p-value is small.

var.test(x = ph_df1$ph_mean, 
         y = ph_df2$ph_mean, 
         alternative = "two.sided")
```
...
meth_df <- inner_join(x = meth_df1, y = meth_df2,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
  rename(meth_mean_wk3 = ch4_mean.x,
         meth_mean_wk1 = ch4_mean.y) %>%
  select(-starts_with("study_week"))

 
t.test(x = meth_df$meth_mean_wk3, y=meth_df$meth_mean_wk1, 
      alternative = "greater", paired = TRUE) 

# we expect the concentration to increase, but my week3 and week1 is in opposite position, so the argument I use is "greater". And the p value is small, suggesting there's significant increase in methane.
```

```{r}
# acetate

ace_df <- inner_join(x = ace_df1, y = ace_df2,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
  rename(ace_mean_wk3 = acetate_mean.x,
         ace_mean_wk1 = acetate_mean.y) %>%
  select(-starts_with("study_week"))

 
t.test(x = ace_df$ace_mean_wk3, y=ace_df$ace_mean_wk1, 
      alternative = "greater", paired = TRUE) 

# we expect the concentration to increase, but my week3 and week1 is in opposite position, so the argument I use is "greater". And the p value is small, suggesting there's significant increase in acetate.
```

```{r}
# propionate
pro_df <- inner_join(x = pro_df1, y = pro_df2,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
  rename(pro_mean_wk3 = propionate_mean.x,
         pro_mean_wk1 = propionate_mean.y) %>%
  select(-starts_with("study_week"))

 
t.test(x = pro_df$pro_mean_wk3, y=pro_df$pro_mean_wk1, 
      alternative = "greater", paired = TRUE) 

# we expect the concentration to increase, but my week3 and week1 is in opposite position, so the argument I use is "greater". the p value is large, suggesting there's no significant increase in propionate.
```

```{r}
# pH
ph_df <- inner_join(x = ph_df1, y = ph_df2,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
  rename(ph_mean_wk3 = ph_mean.x,
         ph_mean_wk1 = ph_mean.y) %>%
  select(-starts_with("study_week"))

 
t.test(x = ph_df$ph_mean_wk3, y=ph_df$ph_mean_wk1, 
      alternative = "less", paired = TRUE) 

# we expect the pH to decrease, but my week3 and week1 is in opposite position, so the argument I use is "less". the p value is large, suggesting there's significant decrease in pH.
```
...
shapiro.test(s7$delta_butyrate)

s8 <- scfa_wide %>%
  filter(supplement_consumed == "none") 
shapiro.test(s8$delta_butyrate)

s9 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium") 
shapiro.test(s9$delta_butyrate)

s10 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium+BRMPS") 
shapiro.test(s10$delta_butyrate)

s11 <- scfa_wide %>%
  filter(supplement_consumed == "transition_HiMaize") 
shapiro.test(s11$delta_butyrate)

s12 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS") 
shapiro.test(s12$delta_butyrate)
...
plot_ace
save_plot(filename = "Lab5/figures/plot_ace_anova.pdf",
          plot = plot_ace)
...
plot_pro
save_plot(filename = "Lab5/figures/plot_pro_anova.pdf",
          plot = plot_pro)
