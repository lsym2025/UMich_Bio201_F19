...
```{r}
scfa_indv$wt_lbs <- na_if(scfa_indv$wt_lbs, 0)
scfa_indv$age <- na_if(scfa_indv$age, 0)
scfa_indv$ht_in <- na_if(scfa_indv$ht_in, 0)
summary(scfa_indv)
...
f18_but_rps <- scfa_indv %>%
  ...
  select(participant_id,sample_number,sample_wt_g,sample_id,butyrate_mM)
colnames(f18_but_rps)
...
mutate(acetate_mmol_kg = round((acetate_mM*0.002)/sample_wt_kg,digits=2),
...
select(-ht_in, -wt_lbs,-sample_wt_g,-acetate_mM,-butyrate_mM,-propionate_mM)
...
mutate(scfa_total_median_mM=acetate_median+butyrate_median+propionate_median) %>% 
mutate(scfa_total_mean_mM=butyrate_mean+acetate_mean+propionate_mean) %>% 
rename(acetate_median_mM = acetate_median, 
                    butyrate_median_mM = butyrate_median,
                    propionate_median_mM = propionate_median) %>% 
rename(acetate_mean_mM = acetate_mean, 
                    butyrate_mean_mM = butyrate_mean,
                    propionate_mean_mM = propionate_mean)
colnames(scfa_wkly_qc)
