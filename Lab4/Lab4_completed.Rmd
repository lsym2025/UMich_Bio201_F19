...
scfa_data %>%
  ...
  summarize(mean_butyrate = mean(butyrate_mmol_kg, na.rm = TRUE), mean_acetate=mean(acetate_mmol_kg, na.rm=TRUE),
...
```{r}
scfa_wkly <- read_delim(file = "Lab3/curated_data/scfa_wkly_qc.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA"))
scfa_wkly %>%
  spread(key = study_week, value = butyrate_mean_mmol_kg)
```
...
```{r}
# write code here
but4_3<-butyrate_wide %>%
 group_by (supplement_consumed) %>%
  summarize(mean_delta_but = mean(delta_butyrate))
      but4_3   
```
...
ace4_3<-acetate_wide %>%
 group_by (supplement_consumed) %>%
  summarize(mean_delta_ace = mean(delta_acetate))
      ace4_3   
```

```{r}
# copy+paste and edit code chunk for propionate
pro4_3<-propionate_wide %>%
 group_by (supplement_consumed) %>%
  summarize(mean_delta_pro = mean(delta_propionate))
      pro4_3  
```
...
ggplot(data = but4_3, 
       mapping = aes(x = supplement_consumed, 
                     y = mean_delta_but) )+
    geom_point() +
    labs(title = "Mean butyrate change for different supplements",
         x = "supplement",
         y = "Fecal butyrate change(mmol/kg)")
...
ggplot(data = ace4_3, 
       mapping = aes(x = supplement_consumed, 
                     y = mean_delta_ace) )+
    geom_point() +
    labs(title = "Mean acetate change for different supplements",
         x = "supplement",
         y = "Fecal acetate change(mmol/kg)")

ggplot(data = pro4_3, 
       mapping = aes(x = supplement_consumed, 
                     y = mean_delta_pro) )+
    geom_point() +
    labs(title = "Mean propionate change for different supplements",
         x = "supplement",
         y = "Fecal propionate change(mmol/kg)")
```
...
# write code here
  ...
  total_scfas <- inner_join(acetate_wide, butyrate_wide,
                ...
                inner_join(propionate_wide,
                          by = c("participant_id", "semester", "supplement_consumed")) %>%
mutate(wk1_total_scfas=ace_wk1+but_wk1+pro_wk1) %>% 
mutate(wk3_total_scfas=ace_wk3+but_wk3+pro_wk3) %>% 
mutate(delta_total_scfas=wk3_total_scfas-wk1_total_scfas)
total_scfas
...
total_scfas %>%
  ...
  gather(key = SCFA_wk, value = SCFA_concentration,
         ace_wk1, ace_wk3, but_wk1, but_wk3, pro_wk1, pro_wk3, wk1_total_scfas, wk3_total_scfas)
```
...
ace_long <- total_scfas %>%
  ...
  select(-starts_with("delta_"), -starts_with("but_"), -starts_with("pro_"), -starts_with("wk1_"),-starts_with("wk3_")) %>% 
...
but_long <- total_scfas %>%
  select(-starts_with("delta_"), -starts_with("ace_"), -starts_with("pro_"), -starts_with("wk1_"),-starts_with("wk3_")) %>% 
...
pro_long<- total_scfas %>%
  select(-starts_with("delta_"), -starts_with("but_"), -starts_with("ace_"), -starts_with("wk1_"),-starts_with("wk3_")) %>% 
...
wkly_means <- scfa_long %>%
  ...
  group_by(participant_id,study_week, scfa,semester,supplement_consumed) %>%
...
# for plots:
save_plot(filename = "Lab4/figures/plot_weekly_mean.pdf",
          plot = p1)
save_plot(filename = "Lab4/figures/plot_scfa_total.pdf",
          plot = plot4_4)
...
pH_qc <- pH_data %>%
  ...
  summarise(wkly_mean_ph=mean(ph)) #calculate weekly average
dim(pH_qc)
```
...
BSS_qc <- BSS_data %>%
  ...
  group_by(participant_id,study_week) %>%
  summarise(wkly_mean_bristol_numeric=mean(bristol_numeric)) #calculate weekly average 
dim(BSS_qc)
```
...
pH_BSS_wkly <- full_join(pH_qc, BSS_qc,
  ...
  mutate(wkly_mean_bristol_numeric = round(wkly_mean_bristol_numeric, digits = 0),
         wkly_mean_ph = round(wkly_mean_ph, digits = 0)) 
#round avgs to reflect sig figs
```
